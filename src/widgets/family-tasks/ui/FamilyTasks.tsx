...
export const FamilyTasks = () => {
  ...
  return (
    <div className="space-y-3">
      <form className="flex gap-gap" onSubmit={submit}>
        <Input
          aria-label="Название новой задачи"
          onChange={(event) => setTitle(event.target.value)}
          placeholder="Добавить семейное дело…"
          value={title}
        />
        <Button
      ...
      <AsyncState
        ...
        loading={<p className="text-muted-text text-sm">Загружаем дела…</p>}
        onRetry={refresh}
      >
        <div className="space-y-2">
          {taskItems
            ...
            .map((task) => (
              <div
                className="border-border bg-surface/50 flex items-center gap-gap rounded-xl border p-2.5"
                key={task.id}
              >
                <button
                  aria-label={task.status === 'COMPLETED' ? 'Вернуть задачу' : 'Завершить задачу'}
                  className="text-acid-green shrink-0"
                  onClick={() =>
                    mutate(() =>
                ...
                <span
                  className={
                    task.status === 'COMPLETED'
                      ? 'text-muted-text flex-1 text-sm line-through'
                      : 'text-text flex-1 text-sm'
                  }
                >
                  {task.title}
                </span>
                <button
                  ...
                  className="text-neon-pink/70 hover:text-neon-pink"
          ...
          {!taskItems.filter((task) => task.status !== 'ARCHIVED').length && (
            <p className="text-muted-text py-2 text-sm">Дел пока нет.</p>
          )}
        </div>
      </AsyncState>
    </div>
