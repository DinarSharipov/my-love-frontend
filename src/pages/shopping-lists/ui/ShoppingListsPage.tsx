    ...
    <main className="h-full overflow-auto pb-24">
      <div className="mx-auto w-full max-w-5xl space-y-5">
        ...
        <AnimatedPanel className="p-5 sm:p-6">
          <form className="flex flex-col gap-3 sm:flex-row sm:items-end" onSubmit={submitList}>
            <Input
              label="Новый список"
              maxLength={150}
        ...
        <AsyncState
          ...
          loading={
            <AnimatedPanel className="text-muted-text p-6">Загружаем покупки…</AnimatedPanel>
          }
          onRetry={refresh}
        >
                ...
                <AnimatedPanel className="min-w-0 p-5" key={list.id}>
                  ...
                  <div className="mt-4 space-y-2">
                    {[...activeItems, ...checkedItems].map((item) => (
                      <article
                        className="border-border bg-elevated/35 flex min-w-0 items-center gap-3 rounded-2xl border p-3"
                        key={item.id}
                      >
                        <button
                          aria-label={
                        ...
                        <div className="min-w-0 flex-1">
                          ...
                          {item.quantity && (
                            <p className="text-muted-text mt-0.5 truncate text-xs">
                              {item.quantity}
                            </p>
                          )}
                        </div>
