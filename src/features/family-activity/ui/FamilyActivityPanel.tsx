    ...
    <AnimatedPanel className="mt-5 p-5 sm:p-6">
      <header className="mb-5 flex items-start justify-between gap-3">
        <div>
          ...
          <p className="text-muted-text mt-1 text-sm">Последние изменения доступны всем участникам семьи.</p>
        </div>
        <Button aria-label="Обновить историю" onClick={() => audit.refetch()} size="s">
          <RefreshCw aria-hidden="true" className="h-4 w-4" />
        </Button>
      </header>
      <AsyncState error={audit.error} errorMessage="Не удалось загрузить историю" hasData={Boolean(audit.data)} isLoading={audit.isLoading} loading={<p className="text-muted-text text-sm">Загружаем историю…</p>} onRetry={audit.refetch}>
        <div className="space-y-2">
          {audit.data?.data.map((event) => (
            <div className="border-border bg-surface/50 rounded-xl border p-3" key={event.id}>
              ...
              <p className="text-muted-text mt-1 text-xs">{event.resourceType} · {formatDateTime(event.createdAt)}</p>
            </div>
          ))}
          {!audit.data?.data.length && <p className="text-muted-text text-sm">История пока пуста.</p>}
        </div>
      </AsyncState>
      <div className="border-border mt-5 flex flex-wrap items-center justify-between gap-3 border-t pt-4">
        <p className="text-muted-text text-xs">Скачайте копию профиля, семейных данных и событий.</p>
