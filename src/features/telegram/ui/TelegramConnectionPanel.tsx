...
export const TelegramConnectionPanel = () => {
  const connection = useGetTelegramConnectionQuery(undefined, {
    pollingInterval: 15_000,
    skipPollingIfUnfocused: true,
  });
  const { refetch } = connection;
  ...
  const connect = async () => {
    ...
    try {
      ...
      setLink(result);
      window.open(
        `${BOT_URL}?start=${encodeURIComponent(result.token)}`,
        '_blank',
        'noopener,noreferrer',
      );
    } catch (requestError) {
  ...
  const linkExpiresAt = useMemo(() => {
    ...
    return Number.isNaN(expiresAt.getTime()) ? undefined : formatDate(expiresAt);
  }, [link]);
  return (
      ...
      <div className="flex items-start justify-between gap-gap">
        <div>
          <h2 className="text-text flex items-center gap-gap font-semibold">
            <Send className="text-cyber-cyan size-5" /> Telegram
          </h2>
          <p className="text-muted-text mt-1 text-xs">
            {active
              ? `Подключён ${new Intl.DateTimeFormat('ru-RU').format(new Date(connection.data!.linkedAt))}`
              : 'Не подключён'}
          </p>
        </div>
        {active ? (
      ...
      {link && !active && (
        <div className="border-border bg-elevated/40 mt-4 rounded-xl border p-3">
          <p className="text-muted-text text-xs">
            Если Telegram не открылся, отправьте боту одноразовый код
            {linkExpiresAt ? ` до ${linkExpiresAt}` : ' в течение 10 минут'}:
          </p>
          ...
          <button
            className="text-muted-text mt-2 text-xs underline"
